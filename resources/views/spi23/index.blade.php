                    ...
                    <div class="table-responsive m-t-40">
                        <table id="dTabel" class="display table table-hover table-striped table-bordered" cellspacing="0" width="100%">
                            <thead>
                                <tr>
                                    <th>No</th>
                                    <th>Unit Kerja Tempat Mengurus Layanan Publik</th>
                                    <th>Unit Eselon I</th>
                                    <th>Nama Layanan yang diterima</th>
                                    <th>Tanggal/ Bulan/ Tahun Mengurus Layanan</th>
                            ...
                            <tbody>
                                ...
                                @else
                                    ...
                                    <tr>
                                            ...
                                            <td>PST BPS Provinsi NTB</td>
                                            <td>Kesestamaan</td>
                                            <td>
                                                @foreach ($item->pLayanan as $layanan)
                                                    {{$layanan->layanan_nama_new}},
                                                @endforeach
                                            </td>
                                            <td>{{ \Carbon\Carbon::parse($item->tanggal)->isoFormat('D/MM/Y')}}</td>
                                            <td><a href="#" class="text-info" data-id="{{$item->tamu_id}}" data-toggle="modal" data-target="#ViewModal">{{$item->tamu->nama_lengkap}}</a></td>
                                            <td>{{$item->tamu->telepon}}</td>
                                            <td>{{$item->tamu->email}} </td>
                                            <td>{{$item->keperluan}}</td>
                                            <td>
                                                @foreach ($item->pFasilitas as $fasilitas)
