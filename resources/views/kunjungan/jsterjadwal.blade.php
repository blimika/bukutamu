<script>
    ...
    $('#tambah_data').on('click', function(e) {
        ...
        //cek hari libur apa tidak/
        if (tgl_kunjungan == "")
        {
            $('#tgl_kunjungan_error').removeClass("has-danger");
            $('#tgl_kunjungan_error').addClass("has-danger");
            Swal.fire({
                type: 'error',
                title: 'error',
        ...
        {
            $.ajax({
        url : '{{route("cek.tanggal","")}}/'+tgl_kunjungan,
        method : 'get',
        cache: false,
        dataType: 'json',
        success: function(data){
            if (data.status == true)
            {
                if (data.hasil.jtgl > 1)
                {
                    ...
                    Swal.fire({
                        type: 'error',
                        title: 'error',
                        text: 'Tanggal '+ tgl_kunjungan +' ('+data.hasil.deskripsi+') bukan hari kerja, pilih tanggal yang lain!'
                        });
                    return false;
                }
                else 
                {
                    $('#tgl_kunjungan_error').removeClass("has-danger");
                    $('#tgl_kunjungan_error').addClass("has-success");
                }
            ...
            else
            {
                ...
                Swal.fire({
                    type: 'error',
                    title: 'error',
                    text: 'Tanggal '+ tgl_kunjungan +'('+data.hasil.status+') bukan hari kerja, pilih tanggal yang lain!'
                    });
                return false;
            }
        },
